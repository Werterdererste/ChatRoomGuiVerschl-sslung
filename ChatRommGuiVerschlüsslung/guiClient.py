...
class guiClient:
    def __init__(self):
        ...
        self.__client = client(self.__root, self.__listbox)
        self.__name = "xx"
    ...
    def Login(self):
        ...
        tk.Label(self.__root, text= "Username").grid(row= 0, column=1, padx=15,
            pady=20)

        username = tk.Entry(self.__root)
        username.grid(row=1, column=1, padx=5,pady=20)
        tk.Button(self.__root, text="Anmelden", command= lambda: self.Anmelden(username)).grid(row=2,column=1, 
            padx=15,
            pady=20,
            ipadx=5,
            ipady=5)
